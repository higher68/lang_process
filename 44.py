        ...
        for line in file_parsed:
            ...
            if line == 'EOS\n':
                ...
                if len(chunks) > 0:
                    ...
                    # items()タプルを生成
                    # タプルは変更不可、リストは書き換え可
                    yield list(sorted_tuple)
                    chunk.clear()

                else:
                    yield []
            # 先頭が*の行は係り受け解析結果なので、Chunkを生成.
            elif line[0] == '*':

                # Chunkのインデックス番号と係り受け先のインデックス番号取得
                cols = line.split(' ')
                idx = int(cols[1])
                ...
                # 要するに、解析結果があったら、前後をみてその情報を入れてるだけか

            # それ以外の行は形態素解析結果なので、Morphを作りChunkに追加
        else:

            # 表層形はtab区切り、それ以外は','区切り。
            cols = line.split('\t')
            res_cols = cols[1].split(',')

            # Morph作成、リストに追加
            chunks[idx].morph.append(
                Morph(
                    cols[0],    # suraface
                    res_cols[6],  # base
                    res_cols[0],  # pos
...
def graph_from_edge_ex(edge_list, directed=False):
...
for chunks in neco_lines():
    ...
    # 描画
    if len(edges) > 0:
        graph = graph_from_edge_ex(edges, directed=True)
        # directed=Trueだと、有向グラフとなり、エッジが矢印になる
