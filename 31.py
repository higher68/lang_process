...
for line in lines:
    for morpheme in line:
        if morpheme['pas'] == '動詞':
            verbs.add(morpheme['surface'])  # 表層系を辞書登録
            verbs_test.append(morpheme['surface'])  # 出現順をカウント


# 確認しやすいように、verbs_testを使って出現順にソートして表示
print(sorted(verbs, key=verbs_test.index))
# sortedのkeyを使った時の流れ
# sort対象の各要素に対して、keyの関数を適用した時の出力を出しておく
# それを用いて並べかえる
# 今回だったら、verbsの各要素に関して、verbs_test内でのindexが何かを調べる。
# 具体的には、verbsが"a"だったら、verbs_test.index("a")となり
# "a"のverbs_test内での位置が出力される
# その処理を全ての要素に関して行う
# その結果を用いてそーとする。
# 以上。
