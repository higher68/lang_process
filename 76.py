...
# 学習データを読み込み予測
with codecs.open(f_sentiment, 'r', fencoding) as file_in, open(f_result,  'w') as file_out:
    for line in file_in:
        # 素性抽出
        data_one_x = extract_features(line[3:], dict_features)

        # 予測、結果出力
        h = hypothesis(data_one_x, theta)   # +1である確率なので、-1と予測された場合には、1-(+1である確率)としたら、-1である確率がでる
        if h > 0.5:
            file_out.write('{}\t{}\t{}\n'.format(line[0:2], '+1', h))
        else:
